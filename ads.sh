...
#ads Control
clear
if [ ! -d "$dizin_else/.$date" ] 
then
${ECHO_PATH} ""
${ECHO_PATH} -e "                             \033[1;36mHoşGeldiniz\033[0m"
${ECHO_PATH} -e "                       \033[1;37mAuroraX Reklam Onay Sistemi\033[0m"
${ECHO_PATH} ""
${ECHO_PATH} -e "\033[1;32m
 http://injectshell.com giriş yapın, 60 saniye bekleyin,
 herhangi bir reklamdan birisine tıklayıp;
 yönlendirdiği linki kopyalayın ve yapıştırıp gönderin.
 Bu işlemi uzun süre yapmamanız durumunda 
 scripte erişiminiz kapatılacaktır\033[0m"
${ECHO_PATH} ""
${ECHO_PATH} -e -n "\033[1;31mReklam URL'si\033[0m:"
read reklamurl
${ECHO_PATH} "  Bağlanılıyor..."
sleep 0.3
if [ -z "$reklamurl" ]
then
      ${ECHO_PATH} " Reklam URL alanı boş bırakılamaz."
	  sleep 2
${ECHO_PATH} ""
${ECHO_PATH} -e "                             \033[1;36mHoşGeldiniz\033[0m"
${ECHO_PATH} -e "                       \033[1;37mAuroraX Reklam Onay Sistemi\033[0m"
...
if [ -z "$reklamurls" ]
then
      ${ECHO_PATH} "Error Code: x502"
      exit
fi
nohup curl --data "islem=reklam&ipalals=$ip&laurl=$reklamurls&kadila=$oturum_kadi" http://sblinux.net/sblinux_post.php >/dev/null 2>&1
$mkdir_path $dizin_else/.$date 
	  exit
fi
nohup curl --data "islem=reklam&ipalals=$ip&laurl=$reklamurl&kadila=$oturum_kadi" http://sblinux.net/sblinux_post.php >/dev/null 2>&1
$mkdir_path $dizin_else/.$date
fi
